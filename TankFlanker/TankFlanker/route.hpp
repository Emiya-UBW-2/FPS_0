...
public:
	main_c() {
		...
		auto Drawparts = std::make_unique<DXDraw>("FPS_0", this->dispx, this->dispy, this->out_dispx, this->out_dispy, settings, FRAME_RATE);	/*汎用クラス*/
		...
		auto Hostpassparts = std::make_unique<HostPassEffect>(this->dispx, this->dispy, settings);												/*ホストパスエフェクト*/
						...
						//プレイヤー操作
						{
							...
							if (settings->useVR_e) {
								...
								if (vrparts->get_hand2_num() != -1) {
									...
									if (ptr_.turn && ptr_.now) {
										if ((ptr_.on[0] & BUTTON_TOUCHPAD) != 0) {
											easing_set(&this->add_pos_buf, (c.mat_HMD.zvec()*ptr_.touch.y() + c.mat_HMD.xvec()*ptr_.touch.x())*-4.f / fps, 0.95f, fps);
										}
										else {
								...
								{
									{
										...
										}
										{
											//身体
											{
												c.body.SetFrameLocalMatrix(c.body_f.first, MATRIX_ref::RotX(c.body_xrad)*MATRIX_ref::RotY(DX_PI_F + c.body_yrad)*MATRIX_ref::Mtrans(c.body_f.second));
												//
											...
											MATRIX_ref m_inv = (MATRIX_ref::RotX(c.body_xrad)*MATRIX_ref::RotY(DX_PI_F + c.body_yrad)).Inverse();
											//右手
											{
												VECTOR_ref vec_a1 = MATRIX_ref::Vtrans(((c.pos + c.pos_RIGHTHAND) - c.body.frame(c.RIGHTarm1_f.first)).Norm(), m_inv);//基準
												...
												MATRIX_ref a2_inv = MATRIX_ref::RotVec2(c.body.frame(c.RIGHThand_f.first) - c.body.frame(c.RIGHTarm2_f.first), MATRIX_ref::Vtrans(((c.pos + c.pos_RIGHTHAND) - c.body.frame(c.RIGHTarm2_f.first)).Norm(), m_inv));
												c.body.SetFrameLocalMatrix(c.RIGHTarm2_f.first, a2_inv*a1_inv.Inverse()*MATRIX_ref::Mtrans(c.RIGHTarm2_f.second));
												//
												c.body.SetFrameLocalMatrix(c.RIGHThand_f.first, m_inv*a2_inv.Inverse()*MATRIX_ref::Mtrans(c.RIGHThand_f.second));
												MATRIX_ref hand_inv = MATRIX_ref::RotVec2(c.body.frame(c.RIGHThand2_f.first) - c.body.frame(c.RIGHThand_f.first), MATRIX_ref::Vtrans(c.mat_RIGHTHAND.zvec()*-1.f, m_inv));
											...
											//左手
											{
												...
												{
													VECTOR_ref vec_a1 = MATRIX_ref::Vtrans(((c.pos + c.pos_LEFTHAND) - c.body.frame(c.LEFTarm1_f.first)).Norm(), m_inv);//基準
													VECTOR_ref vec_a1L1 = VECTOR_ref(VGet(0.f, -1.f, vec_a1.y() / vec_a1.z())).Norm();//x=0とする
													VECTOR_ref vec_a1L2 = VECTOR_ref(VGet(-1.f, 0.f, vec_a1.x() / vec_a1.z())).Norm();//y=0とする
													...
													MATRIX_ref a2_inv = MATRIX_ref::RotVec2(c.body.frame(c.LEFThand_f.first) - c.body.frame(c.LEFTarm2_f.first), MATRIX_ref::Vtrans(((c.pos + c.pos_LEFTHAND) - c.body.frame(c.LEFTarm2_f.first)).Norm(), m_inv));
													c.body.SetFrameLocalMatrix(c.LEFTarm2_f.first, a2_inv*a1_inv.Inverse()*MATRIX_ref::Mtrans(c.LEFTarm2_f.second));
													//
													c.body.SetFrameLocalMatrix(c.LEFThand_f.first, m_inv*a2_inv.Inverse()*MATRIX_ref::Mtrans(c.LEFThand_f.second));
													MATRIX_ref hand_inv = MATRIX_ref::RotVec2(c.body.frame(c.LEFThand2_f.first) - c.body.frame(c.LEFThand_f.first), MATRIX_ref::Vtrans((c.obj.frame(c.ptr_now->frame[6].first) - c.obj.frame(c.ptr_now->frame[6].first + 1)).Norm(), m_inv));
								...
								//移動
								{
									...
									if (wkey) {
										easing_set(&this->add_pos_buf, c.mat_HMD.zvec()*-(runkey ? 8.f : 4.f) / fps, 0.95f, fps);
									}
									if (skey) {
										easing_set(&this->add_pos_buf, c.mat_HMD.zvec()*(runkey ? 8.f : 4.f) / fps, 0.95f, fps);
									}
									if (akey) {
										easing_set(&this->add_pos_buf, c.mat_HMD.xvec()*(runkey ? 8.f : 4.f) / fps, 0.95f, fps);
									}
									if (dkey) {
										easing_set(&this->add_pos_buf, c.mat_HMD.xvec()*-(runkey ? 8.f : 4.f) / fps, 0.95f, fps);
									}
								...
								//pos
								{
									...
									MATRIX_ref m_inv = (MATRIX_ref::RotX(c.body_xrad)*MATRIX_ref::RotY(DX_PI_F + c.body_yrad));
									{
										...
										for (size_t i = 0; i < c.gun_slot.size(); i++) {
											if (c.gun_slot[i].ptr != nullptr) {
												c.gun_slot[i].obj.SetMatrix(MATRIX_ref::RotY(-DX_PI_F)*MATRIX_ref::RotX(DX_PI_F / 2)
													*m_inv
													* MATRIX_ref::Mtrans(MATRIX_ref::Vtrans(VGet((float(i) - float(c.gun_slot.size()) *0.5f)*0.1f, -0.15f, 0.3f), m_inv) + (c.pos_HMD + c.pos))
												);
											}
										}
									}
									...
									{
										c.body.SetMatrix(c.mat*MATRIX_ref::Mtrans(c.pos));
										c.pos_HMD = (c.body.frame(/*c.body_f.first*/21) + (c.body.frame(/*c.body_f.first*/19) - c.body.frame(/*c.body_f.first*/21))*0.5f) - c.pos;
									}
									//右手
									{
										...
										if (this->ads.first) {
											easing_set(&this->gunpos_TPS, VGet(-0.035f, 0.f - pv.y(), -0.25f), 0.75f, fps);
											...
											easing_set(&this->gunpos_TPS, VGet(-0.125f, -0.075f - pv.y(), -0.3f), 0.75f, fps);
...
break;
						...
						if (settings->useVR_e) {
							...
							this->camvec = chara[id_mine].mat_RIGHTHAND.zvec()*-1.f;
							this->camup = chara[id_mine].mat_RIGHTHAND.yvec();
						}
