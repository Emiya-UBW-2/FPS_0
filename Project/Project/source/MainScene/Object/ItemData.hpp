...
namespace FPS_n2 {
	namespace Sceneclass {
		...
		public:
			...
			const auto&	GetXsize(void) const noexcept { return this->Is90 ? this->m_ysize : this->m_xsize; }
			const auto&	GetYsize(void) const noexcept { return this->Is90 ? this->m_xsize : this->m_ysize; }
		public:
			void Set(const std::shared_ptr<ItemData>& data) {
				Data = data;
				Data->GetSlotPic().GetSize(&m_xsize, &m_ysize);
				m_xsize /= 64;
				m_ysize /= 64;
			}
			void Rotate() {
			...
			void Draw(int xp, int yp) {
				Data->GetSlotPic().DrawRotaGraph(xp + GetXsize()*y_r(64) / 2, yp + GetYsize()*y_r(64) / 2, (float)y_r(64) / 64.f, deg2rad(this->Is90 ? 90.f : 0.f), false);
			}
		};
	};
};
